...
session.headers.update({
    "User-Agent": "LF-ADP-Agent/1.0 (mailto:curry13145@gmail.com)"
})
...
def arxiv_search_tool(query: str, max_results: int=5) -> dict:
    ...
    try:
        ...
        return {"error": f"Parsing failed: {str(e)}", "results": []}
...
def tavily_search_tool(query: str, max_results: int = 5, include_images: bool = False) -> dict:
    """
    ...
    Returns:
        dict: A dictionary with 'results' key containing search results.
    """
    api_key = os.getenv("TAVILY_API_KEY")
    if not api_key:
        raise ValueError("TAVILY_API_KEY not found in environment variables.")

    # Initialize Tavily client (standard version doesn't support api_base_url)
    ...
    try:
        ...
        if include_images:
            ...
            return {"results": results, "images": images, "count": len(results)}

        return {"results": results, "count": len(results)}

    except Exception as e:
        return {"error": str(e), "results": []}  # For LLM-friendly agents
...
def wikipedia_search_tool(query: str, sentences: int = 5) -> dict:
    """
    ...
    Returns:
        dict: A dictionary containing title, summary, and URL.
    """
    try:
